...
def scrape_wildberries(browser, query):
    ...
    try:
        ...
        for p in products:
            p['price_num'] = int(clean_number(p['price'])) if clean_number(p['price']) else 0
        return products
...
def scrape_yandex(browser, query):
    ...
    try:
        ...
        for p in products:
            ...
            p['price_num'] = int(cleaned) if cleaned else 0
        return products
    except Exception as e:
        print(f'  Error: {e}')
        return []
...
def scrape_aliexpress(browser, query):
    ...
    try:
        ...
        for item in raw:
            ...
            # Цена - ищем число перед ₽
            price = None
            price_matches = re.findall(r'(\d[\d\s]*)\s*₽', text)
            for pm in price_matches:
                cleaned = clean_number(pm)
                if cleaned:
                    num = int(cleaned)
                    if 15000 < num < 300000:
                        price = num
            ...
            for line in text.split('\n'):
                l = line.strip()
                if 'iphone' in l.lower() and len(l) > 25 and '₽' not in l and 'СКИДКА' not in l:
                    title = l[:100]
                    break
...
if __name__ == '__main__':
    ...
    # Лучшая цена (только реальные айфоны > 30000)
