...
def filter_products(products):
    exclude = {'чехол', 'кейс', 'стекло', 'пленка', 'кабель', 'зарядк', 'ремешок', 'наушники'}
...
def scrape_wildberries(query):
    """WB - как в рабочем парсере с GitHub"""
    try:
        headers = {
        ...
        for p in data.get('data', {}).get('products', [])[:20]:
            # Цена из sizes как в рабочем парсере
            sale_price = None
            if 'sizes' in p and len(p['sizes']) > 0:
                price_info = p['sizes'][0].get('price', {})
                sale_price = price_info.get('product')
                if sale_price:
                    sale_price = int(sale_price / 100)
            
            # Fallback на salePriceU
            if not sale_price:
...
def scrape_yandex(query):
    try:
        ...
        r = requests.get(f'https://market.yandex.ru/search?text={query}', headers=headers, timeout=15)
