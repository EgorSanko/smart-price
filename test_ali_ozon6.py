...
def clean_number(s):
    """Убирает все виды пробелов и нечисловые символы"""
    return re.sub(r'\D', '', s)
...
with sync_playwright() as p:
    ...
    for item in raw:
        ...
        if title and price and title not in seen:
            seen.add(title)
            ozon.append({'title': title, 'price': f'{price:,} '.replace(',', ' '), 'price_num': price, 'url': item['url'], 'marketplace': 'ozon'})
    ...
    raw = page.evaluate("""() => {
        ...
        links.forEach(link => {
            ...
            data.push({ url: url, text: card ? card.innerText : '' });
        });
    ...
    for item in raw:
        ...
        if title and price and title not in seen:
            seen.add(title)
            ali.append({'title': title, 'price': f'{price:,} '.replace(',', ' '), 'price_num': price, 'url': item['url'], 'marketplace': 'aliexpress'})
