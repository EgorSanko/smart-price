...
def ai_normalize(q):
    try:
        with GigaChat(credentials=GIGACHAT_KEY, verify_ssl_certs=False) as giga:
            prompt = f'''Исправь запрос: "{q}"
Примеры: "посо фб 256" -> "Poco F6 256GB", "айфон 16" -> "iPhone 16", "редми ноут 13" -> "Redmi Note 13"
Верни JSON: {{"normalized_query": "..."}}'''
            r = giga.chat(prompt)
...
Оставь ТОЛЬКО подходящие (убери чехлы, аксессуары). Верни JSON: {{"matching": [товары], "excluded": [{{"title":"...", "reason":"..."}}], "best_price": {{дешевый}}, "summary": "..."}}'''
            r = giga.chat(prompt)
            text = re.sub(r'^```json?\s*|\s*```$', '', r.choices[0].message.content.strip())
            return json.loads(text)
...
def search_stream():
    ...
    def gen():
        yield f"data: {json.dumps({'step': 'ai', 'message': 'AI анализирует запрос...'}, ensure_ascii=False)}\n\n"
        ...
        yield f"data: {json.dumps({'step': 'scrape', 'message': 'Ищем на WB + Яндекс + Ozon параллельно...'}, ensure_ascii=False)}\n\n"
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for future in as_completed(futures):
                ...
                results[name] = items
                all_p.extend(items)
                mp_name = {'wb': 'Wildberries', 'ym': 'Яндекс', 'oz': 'Ozon'}[name]
                yield f"data: {json.dumps({'step': 'found', 'message': f'{mp_name}: {len(items)} товаров'}, ensure_ascii=False)}\n\n"

        yield f"data: {json.dumps({'step': 'filter', 'message': 'AI фильтрует...'}, ensure_ascii=False)}\n\n"
...
def chat():
    ...
    try:
        with GigaChat(credentials=GIGACHAT_KEY, verify_ssl_certs=False) as giga:
            r = giga.chat(f"Ты Фиксик-помогатор для шоппинга. Кратко на русском.\n\n{msg}")
...
if __name__ == '__main__':
    print('Smart Price API v8 (Parallel)')
